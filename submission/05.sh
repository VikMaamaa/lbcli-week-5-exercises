# Create a CSV script that would lock funds until one hundred and fifty blocks had passed
# publicKey=02e3af28965693b9ce1228f9d468149b831d6a0540b25e8a9900f71372c11fb277
#!/bin/bash

# Create a CSV (CheckSequenceVerify) script that locks funds until 150 blocks have passed
public_key=02e3af28965693b9ce1228f9d468149b831d6a0540b25e8a9900f71372c11fb277

# Compute HASH160 (SHA256 followed by RIPEMD160) of the public key
pubkey_hash=$(echo -n "$public_key" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -ripemd160 -binary | xxd -p -c 20)

# 150 blocks = 0x0096, converted to little-endian = 9600
blocks_little_endian="9600"

# Construct and output the CSV script in hex:
# 02          = push 2 bytes (length of block count)
# <blocks>    = relative lock time (150 blocks) in little-endian
# b2          = OP_CHECKSEQUENCEVERIFY
# 75          = OP_DROP
# 76          = OP_DUP
# a9          = OP_HASH160
# 14          = push 20 bytes (length of pubkey hash)
# <pubkeyhash>= HASH160 of the public key
# 88          = OP_EQUALVERIFY
# ac          = OP_CHECKSIG
echo "02${blocks_little_endian}b27576a914${pubkey_hash}88ac"